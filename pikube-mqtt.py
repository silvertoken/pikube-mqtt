...
mqtt_crd = kclient.V1CustomResourceDefinition(
    api_version="apiextensions.k8s.io/v1",
    kind="CustomResourceDefinition",
    metadata=kclient.V1ObjectMeta(name="mqtt.operators.silvertoken.github.io"),
...
try:
	api.create_custom_resource_definition(mqtt_crd)
...
def configure(settings: kopf.OperatorSettings, **_):
    settings.peering.name = "mqtt"
    settings.peering.mandatory = True
    
@kopf.on.create('operators.silvertoken.github.io', 'v1', 'mqtt')
def on_unifi_create(namespace, spec, body, **kwargs):
	logging.debug(f"Mqtt create handler is called: {body}")
	...
	if len(depList.items) == 0:
		logging.info(f"Creating a new mqtt deployment in namespace '{namespace}' with name: {body.metadata.name}")
		dep = gen_mqtt_deployment(namespace, body.metadata.name, spec)
	...
	if len(srvList.items) == 0:
		logging.info(f"Creating a new mqtt service in namespace '{namespace}' with name: {body.metadata.name}")
		srv = gen_mqtt_service(namespace, body.metadata.name, spec)
	...
	logging.info("Checking DNS records for mqtt...")
 
	deploy_mqtt_dns(namespace, body.metadata.name, spec)
				

def deploy_mqtt_dns(namespace, name, spec):
	...
	if len(dnsList['items']) == 0:
		logging.info(f"Creating a new mqtt DNS record in namespace '{namespace}' with name: {name}")
		body = {
...
def gen_mqtt_deployment(namespace, name, spec):
			...
			template = kclient.V1PodTemplateSpec(
				...
				spec =  kclient.V1PodSpec(
					containers = [kclient.V1Container(
						...
						ports = [
							kclient.V1ContainerPort(container_port=1883, name="mqtt")
						],
						volume_mounts = [
